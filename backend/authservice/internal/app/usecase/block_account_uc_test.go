...
func TestBlockAccountUC_Execute(t *testing.T) {
	type adapter struct {
	...
	var tests = []testCase{
		...
		{
			name: "Failure - cannot block admin",
			mockBehaviour: func(a adapter, acc *model.Account) {
				a.account.EXPECT().
					GetByID(mock.Anything, acc.ID()).
					Return(acc, nil)
			...
			expectErr: ucerrs.ErrCannotBlock,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			uc := usecase.NewBlockAccountUC(accountRepo, accountRoleRepo)
